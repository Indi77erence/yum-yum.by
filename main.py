...
async def get_page_data(session, market):
    async with session.get(url=market) as response:
        ...
        if market == 'https://woksushi.by/':
            ...
            content_action_list = (content.text.replace('\n', '') for content in content_action)

            image_action = soup.find_all('img', class_='attachment-woocommerce_thumbnail size-woocommerce_thumbnail')
            ...
            data_pizzahome = zip(name_action_list, content_action_list, price_action_gen,
